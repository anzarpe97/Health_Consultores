...
class AccountMove(models.Model):
    ...
    def create(self, values_list):
        """
        Crea los movimientos contables y valida que la fecha contable
        no sea menor que la fecha de la factura.
        """
    ...
    def write(self, vals):
        # La validación de partner_id y fecha debe estar dentro del bucle
        # para usar 'move_account' en lugar de 'self'.
        for move_account in self:
            # AQUI DEBE IR TODA LA LÓGICA DE VALIDACIÓN y MANEJO DE PO
            # que antes tenías en tu código, usando 'move_account'
            
            # Ejemplo:
            if move_account.move_type == 'in_invoice' and move_account.invoice_origin:
                # USAR move_account.invoice_origin AQUÍ
                order_purchase = self.env['purchase.order'].search([('name', '=', move_account.invoice_origin)], limit=1)
                if order_purchase:
                    vals.update({'partner_id': order_purchase.partner_id.id})
            
            # B. Lógica de validación de campos que pueden estar en 'vals' o en 'move_account'
            partner_id = vals.get('partner_id') or move_account.partner_id.id
            if partner_id:
            ...
            # C. Lógica de validación de fechas
            fecha_factura = False
            ...
            # Usar el valor existente del registro si no está en 'vals'
            elif move_account.invoice_date:
                fecha_factura = move_account.invoice_date
            ...
            # Usar el valor existente del registro si no está en 'vals'
            elif move_account.date:
                fecha = move_account.date

            if fecha and fecha_factura:
                if fecha_factura > fecha:
                    raise UserError(_('La fecha contable no puede ser menor a la fecha de la factura'))

        # 2. Lógica de pre-escritura que puede o no estar dentro del bucle (depende de la necesidad de iteración)
        
        # Estas validaciones se refieren a toda la colección 'self' y a los 'vals' recibidos
        ...
        if vals.get('supplier_invoice_number', False):
            # El método _unique_invoice_per_partner debe manejar la unicidad correctamente
            # sobre la colección 'self' o debe ser llamado dentro del bucle anterior.
            # Asumo que la validación aquí es correcta, pero el return es incorrecto en write.
            ...
            if not supplier_invoice_number_id:
                # 💥 Nota: El 'return' con diccionario de UserError NO funciona en el ORM 'write'. Debe ser 'raise'.
                raise UserError("Advertencia! \nEl Numero de la Factura del Proveedor ya Existe")
        
        # Eliminar el campo de contexto antes de llamar a super
        if vals.pop('check_fiscal', None):
            pass

        # 3. Llamar a la cadena de herencia (super)
