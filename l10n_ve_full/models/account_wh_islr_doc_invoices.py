...
class AccountWhIslrDocInvoices(models.Model):
    ...
    def _get_xml_lines(self, ail_brw):
        ...
        return {
            ...
            'partner_id': acc_part_id.id,  # UserError Depends if is a customer
                                           # or supplier
            'concept_id': ail_brw.concept_id.id,
            'partner_vat': document_v[0:12] if document_v else str(),  # UserError Depends if is a
                                              # customer or supplier
            'porcent_rete': 0.0,  # To be updated later

            'control_number': ail_brw.move_id.correlative,
